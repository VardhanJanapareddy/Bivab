...
function bluebell_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('bluebell_redux_custom_fonts_load', 'bluebell_redux_custom_fonts_load');
function bluebell_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = bluebell_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/bluebell/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [bluebell_social_share_output description]
 *
 ...
 */
function bluebell_social_share_output( $icon, $color = false ) {
...
function bluebell_get_social_icons()
{
	...
	if( bluebell_set( $options, 'social_media' ) && is_array( bluebell_set( $options, 'social_media' ) ) )
	{
		$total = count( bluebell_set( $options, 'social_media' ) ) - 2;
		foreach( bluebell_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = bluebell_set( $social_icon, 'title');
			$class = bluebell_set( $social_icon, 'class');
			$link = bluebell_set( $social_icon, 'social_link');
			$icon = bluebell_set( $social_icon, 'social_icon');
...
add_action('bluebell_social_share_output_two', 'bluebell_social_share_output_two');
...
<div class="blog-post-share-icon">
    ...
    <ul class="social-icon">
        ...
        <li class="facebook"><a href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>" target="_blank">Facebook</a></li>
        <?php endif;?>
    
        <?php if($options->get( 'twitter_sharing' )):?>
        <li class="twitter"><a href="https://twitter.com/share?url=<?php echo esc_url(get_permalink($PostID)); ?>&text=<?php echo esc_attr($post_slug=$PostName); ?>" target="_blank">Twitter</a></li>
        <?php endif;?>
    
        <?php if($options->get( 'linkedin_sharing' )):?>
        <li class="linkedin"><a href="http://www.linkedin.com/shareArticle?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>">Linkedin</a></li>
        <?php endif;?>
    
        <?php if($options->get( 'pinterest_sharing' )):?>
        <li class="facebook"><a href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo esc_url(get_permalink($PostID)); ?>&description=<?php echo esc_attr($post_slug=$PostName); ?>">Pinterest</a></li>
        <?php endif;?>
    
        <?php if($options->get( 'reddit_sharing' )):?>
        <li class="google-plus"><<a href="http://reddit.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>">Reddit</a></li>
        <?php endif;?>
    
        <?php if($options->get( 'tumblr_sharing' )):?>
        <li class="twitter"><a href="http://www.tumblr.com/share/link?url=<?php echo esc_url(get_permalink($PostID)); ?>&name=<?php echo esc_attr($post_slug=$PostName); ?>">Tumblr</a></li>
        <?php endif;?>
    
        <?php if($options->get( 'digg_sharing' )):?>
        <li class="linkedin"><a href="http://digg.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>">Digg</a></li>
        <?php endif;?>
...
function bluebell_get_social_icon()
{
    ...
    <?php $icons = $options->get( 'icons_social_share' );
		...
		<ul class="social-links">
			...
			<li><a target="_blank" href="<?php echo esc_url(bluebell_set( $header_social_icons, 'url' )); ?>" style="background-color:<?php echo esc_attr(bluebell_set( $header_social_icons, 'background' )); ?>; color: <?php echo esc_attr(bluebell_set( $header_social_icons, 'color' )); ?>"><i class="fa <?php echo esc_attr( bluebell_set( $header_social_icons, 'icon' ) ); ?>"></i></a></li>
            <?php endforeach; ?>
		</ul>
	<?php endif; ?>
<?php }
 ...
 */
function bluebell_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'bluebell_unset_default_widgets_args' );
 ...
 */
function bluebell_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'bluebell_child_functions_php', 10, 2 );
 ...
 */
function bluebell_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'bluebell'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'bluebell_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'bluebell' ),
			'preview_url'              => 'http://fastwpdemo.com/newwp/bluebell/',
		),
	);
}

add_filter( 'merlin_import_files', 'bluebell_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function bluebell_after_import_setup() {
	...
	$blog_page_id  = get_page_by_title( 'Blog With Sidebar' );
...
add_action( 'merlin_after_all_import', 'bluebell_after_import_setup' );
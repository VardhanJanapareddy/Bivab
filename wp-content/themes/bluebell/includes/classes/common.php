...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => bluebell_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => bluebell_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => bluebell_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => bluebell_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => bluebell_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => bluebell_meta( 'banner_elementor_template', $page_id ),
				'banner'            => bluebell_set( bluebell_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => bluebell_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => bluebell_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : bluebell_the_title( $template ),
				'banner'        => bluebell_set( $bg, 'url' ),

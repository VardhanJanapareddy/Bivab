...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'bluebell' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
		);

		$scripts = apply_filters( 'BLUEBELL/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( BLUEBELL_NONCE ),
		);

		wp_localize_script( 'jquery', 'bluebell_data', $header_data );

		if ( bluebell_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', bluebell_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'BLUEBELL/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "bluebell-{$name}", $style );
			} else {
				wp_enqueue_style( "bluebell-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = bluebell_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= bluebell_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'bluebell-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'bluebell-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$data = \BLUEBELL\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'bluebell' . '_options-mods' );

		if ( $custom_font = bluebell_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('bluebell_redux_custom_fonts_load', $custom_font );
